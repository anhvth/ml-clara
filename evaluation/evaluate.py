...
class EvaluationMetrics:
    ...
    def entity_preserve_metric_by_category(
        self, prediction_tokens: list[list[str]], reference_docs: list[str]
    ) -> dict[str, float]:
...
class ResultCalculator:
    ...
    def calculate_basic_metrics(self, result_list: list[dict]) -> dict[str, float]:
        ...
        for result in result_list:
            ...
            ground_truth = (
                result[answer_key][0] if answer_key == "golden_answers" else result[answer_key]
            )
    ...
    def calculate_stage2_metrics(
        self, result_list: list[dict], k_values: list[int] = [1, 3, 5]
    ) -> dict[str, float]:
        """Calculate stage2 metrics with recall and precision."""
        basic_metrics = self.calculate_basic_metrics(result_list)

        recall = dict.fromkeys(k_values, 0)
        precision = dict.fromkeys(k_values, 0)
    ...
    def calculate_paraphrase_metrics(self, result_list: list[dict]) -> dict[str, float]:
        ...
        for result in result_list[:2000]:
            ...
            bs = self.metrics.bertscore.compute(
            ...
            seen_metrics["entity_preserve"] += self.metrics.entity_preserve_metric(
        ...
        for result in result_list[2000:]:
            ...
            unseen_metrics["entity_preserve"] += self.metrics.entity_preserve_metric(
                prediction, ground_truth
            )
    ...
    def visualize_mse(self, result_list: list[dict], save_path: str) -> dict[str, Any]:
        ...
        ax.scatter(
            ...
            edgecolors="white",
            linewidth=0.5,
            label="Document Representations",
            zorder=2,
        )
        ...
        ax.scatter(
            ...
            edgecolors="white",
            linewidth=0.5,
            label="Memory Tokens Representations",
            zorder=3,
        )
        ...
        plt.savefig(
            os.path.join(save_path, "tsne_visualization_scientific.png"),
            dpi=300,
            bbox_inches="tight",
            facecolor="white",
        )
        ...
        distances = np.array(
            [np.linalg.norm(mem_reps[i] - non_mem_reps[i]) for i in range(len(mem_reps))]
        )
...
class AcceleratedCLaRaInference:
    ...
    def __init__(
        self, model_path: str, training_stage: str = None, generation_top_k: int = None, args=None
    ):
        self.args = args
        ...
        with torch.no_grad():
            try:
                if training_stage == "stage2":
                    return self._process_stage2(
                        model, batch_questions, batch_documents, stage2_mips, time_count
                    )
        ...
        if time_count:
            ...
            if stage2_mips:
                batch_out_normal, topk_idx = model.generate_from_questions(
                    questions=batch_questions, max_new_tokens=64, stage2_mips=stage2_mips
                )
            else:
                batch_out_normal, topk_idx = model.generate_from_questions(
                    questions=batch_questions,
    ...
    def _process_stage1(self, model, batch_questions, batch_documents):
        ...
        for docs, question in zip(batch_documents, batch_questions):
            ...
            out_compressed = model.generate_from_compressed_documents_and_questions(
                questions=[question], compressed_documents=embeddings
            )
    ...
    def _process_paraphrase(self, model, batch_questions, batch_documents):
        ...
        for docs, question in zip(batch_documents, batch_questions):
            out_compressed = model.generate_from_paraphrase(
                questions=["" for _ in range(len(docs))], documents=[docs]
            )
            batch_out_compressed.extend(out_compressed)
            torch.cuda.empty_cache()
...
def main():
    ...
    parser.add_argument(
        "--dataset", type=str, default="musique", help="Comma-separated list of datasets"
    )
    parser.add_argument("--gold_retrieval", action="store_true", help="Use gold retrieval context")
    ...
    for dataset in datasets_list:
        ...
        model_path = os.path.join(
            "/mnt/task_wrapper/user_output/artifacts/data/train_checkpoint", args.model_path
        )
        ...
        with inference_engine.accelerator.split_between_processes(
            processed_data, apply_padding=False
        ) as local_data:
            print(
                f"Process {inference_engine.accelerator.process_index}: processing {len(local_data)} samples"
            )
            ...
            for batch_idx in tqdm(
                ...
                batch_documents = (
                    [item["documents"] for item in batch] if "documents" in batch[0] else None
                )
                batch_answers = (
                ...
                # Prepare results
                batch_results = []
                for i, (item, normal_out, compressed_out) in enumerate(
                    zip(batch, batch_out_normal, batch_out_compressed)
                ):
        ...
        if args.efficient_count and inference_engine.accelerator.is_main_process:
            eff_dic = {
                "compress_time_ms": round(
                    (time_count_dic["compress_time"] / time_count_dic["count"]) * 1000, 2
                ),
                "query_time_ms": round(
                    (time_count_dic["query_time"] / time_count_dic["count"]) * 1000, 2
                ),
                "generate_time_ms": round(
                    (time_count_dic["generate_time"] / time_count_dic["count"]) * 1000, 2
                ),
                "total_time_ms": round(
                    (time_count_dic["total_time"] / time_count_dic["count"]) * 1000, 2
                ),
                "sample_count": time_count_dic["count"],
            }
            eff_output_path = os.path.join(
                model_path,
                f"efficiency_{dataset}_{args.stage}_{args.gold_retrieval}_{args.generation_top_k}.json",
            )
        ...
        if inference_engine.accelerator.is_main_process:
            ...
            output_path = os.path.join(
                model_path,
                f"{dataset}_{args.stage}_{args.gold_retrieval}_{args.generation_top_k}.jsonl",
            )
    ...
    if len(all_results_metrics) > 0:
        metrics_path = os.path.join(
            model_path,
            f"results_metrics_{args.stage}_{args.gold_retrieval}_{args.generation_top_k}.json",
        )
