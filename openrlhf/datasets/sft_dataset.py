...
def make_collate_fn(
    clara_model, enc_max_len: int = 256, dec_max_len: int = 1024, qa_loss: bool = False
):
    """
    ...
    def _mask_information_spans(
        labels: torch.Tensor, input_ids: torch.Tensor, attention_mask: torch.Tensor, tokenizer
    ) -> torch.Tensor:
    ...
    def _process_reasoning_batch(batch_data: tuple) -> dict[str, Any]:
        ...
        for answer in answers:
            ...
            pattern_full = (
                r"<(?:information|think|answer|search)>.*?</(?:information|think|answer|search)>"
            )
            tags = re.findall(
                r"<(information|think|answer|search)>.*?</\1>", answer, flags=re.DOTALL
            )
            ...
            for key, value in thinking_path.items():
                if "information" in key:
                    ...
                    info_match = re.search(
                        r"<information>(.*?)</information>", value, flags=re.DOTALL
                    )
                    if info_match:
        ...
        prompt_responses = [
            clara_model._blend_prompt_and_selected_memory_tokens_for_reasoning(query=q, answer=tp)
    ...
    def collate(batch: list[tuple]) -> dict[str, Any]:
        ...
        if clara_model.training_stage not in [
            "stage2",
...
) -> tuple[list[str], str, str, str, list[int]]:
    """
...
class SFTDataset(Dataset):
    ...
    def _process_data(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.multiturn:
            assert not self.output_key or not data[self.output_key], (
            ...
            for idx, message in enumerate(data[input_key]):
                if message["role"] == "assistant":
                    prompt = apply_chat_template(
                        data[input_key][:idx], tokenize=False, add_generation_prompt=True
                    )
                    response = apply_chat_template(data[input_key][: idx + 1], tokenize=False)[
                        len(prompt) :
                    ]
