...
def setup_datasets(args: argparse.Namespace, tokenizer, strategy, model: CLaRa):
    ...
    train_dataloader = strategy.setup_dataloader(
        ...
        collate_fn=make_collate_fn(
            model, qa_loss=args.qa_loss, dec_max_len=args.max_len, enc_max_len=args.doc_max_length
        ),
    )
...
def train(args: argparse.Namespace):
    ...
    tokenizer = get_tokenizer(
        args.pretrain, model, "right", strategy, use_fast=not args.disable_fast_tokenizer
    )
...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    model_group.add_argument(
        "--pure_inference", action="store_true", default=False, help="Pure inference mode"
    )
    ...
    clara_group.add_argument(
        "--compress_rate", type=int, default=32, help="Document compression rate"
    )
    clara_group.add_argument(
        "--qa_loss", action="store_true", default=True, help="Use QA loss for joint training"
    )
    clara_group.add_argument(
        "--stage2_mips", action="store_true", default=False, help="Use MIPS for stage2 retrieval"
    )
    clara_group.add_argument(
        "--stage2_retrieval_top_n", type=int, default=1, help="Top-n documents for stage2 retrieval"
    )
    clara_group.add_argument(
        "--mse_loss",
        action="store_true",
    ...
    checkpoint_group.add_argument(
        "--save_hf_ckpt", action="store_true", default=False, help="Save HuggingFace checkpoint"
    )
    checkpoint_group.add_argument(
        "--disable_ds_ckpt", action="store_true", default=False, help="Disable DeepSpeed checkpoint"
    )
    checkpoint_group.add_argument(
        "--ckpt_path", type=str, default="./ckpt/checkpoints_sft", help="Checkpoint path to load"
    )
    checkpoint_group.add_argument(
        "--load_checkpoint", action="store_true", default=False, help="Load from checkpoint"
    )
    checkpoint_group.add_argument(
        "--max_ckpt_num", type=int, default=3, help="Max checkpoint number"
    )
    checkpoint_group.add_argument(
        "--max_ckpt_mem", type=int, default=1e8, help="Max checkpoint memory"
    )
    ...
    training_group.add_argument(
        "--adam_betas", type=float, nargs=2, default=(0.9, 0.95), help="Adam optimizer betas"
    )
    training_group.add_argument("--max_norm", type=float, default=1.0, help="Gradient clipping")
    training_group.add_argument(
        "--pretrain_mode", action="store_true", default=False, help="Use pretrain loss"
    )
    ...
    distributed_group.add_argument(
        "--micro_train_batch_size", type=int, default=8, help="Batch size per GPU"
    )
    distributed_group.add_argument(
    ...
    distributed_group.add_argument(
        "--flash_attn", action="store_true", default=False, help="Enable FlashAttention2"
    )
    distributed_group.add_argument(
        "--ds_tensor_parallel_size", type=int, default=1, help="DeepSpeed Tensor parallel size"
    )
    # Dataset configuration
    dataset_group = parser.add_argument_group("Dataset Configuration")
    dataset_group.add_argument("--dataset", type=str, required=True, help="Training dataset path")
    dataset_group.add_argument(
        "--dataset_probs", type=str, default=None, help="Dataset sampling probabilities"
    )
    dataset_group.add_argument(
        "--eval_dataset", type=str, default=None, help="Evaluation dataset path"
    )
    ...
    dataset_group.add_argument(
        "--max_samples", type=int, default=1000000, help="Maximum samples to use"
    )
    ...
    misc_group.add_argument("--use_ms", action="store_true", default=False, help="Use ModelScope")

    # Debug arguments for color-coded confidence visualization
    debug_group = parser.add_argument_group("Debug Configuration")
    debug_group.add_argument(
        "--debug_mode",
        action="store_true",
        default=False,
        help="Enable debug mode with color-coded token confidence output",
    )
    debug_group.add_argument(
        "--debug_every_steps", type=int, default=10, help="Show debug output every N steps"
    )
    debug_group.add_argument(
        "--debug_max_print_tokens",
        type=int,
        default=100,
        help="Maximum number of tokens to print in debug output",
    )
...
def validate_arguments(args: argparse.Namespace):
    ...
    if args.doc_max_length % args.compress_rate != 0:
        print(
            f"Warning: doc_max_length ({args.doc_max_length}) is not divisible by "
