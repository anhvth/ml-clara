...
class Converter(nn.Module):
    ...
    def _print_trainable_parameters(self):
        ...
        print(
            f"Converter trainable parameters: {trainable_params}, Total parameters: {total_params}"
        )
...
def remote_generate(docs: list[str], questions: list[str], api_url: str) -> list[str]:
    ...
    response = requests.post(f"{api_url}/generate", json={"docs": docs, "questions": questions})
    return response.json()["texts"]


def add_memory_tokens_to_inputs(
    input_ids: torch.Tensor, attention_mask: torch.Tensor, n_mem_tokens: int, tokenizer
...
class CLaRa(PreTrainedModel):
    ...
    def _create_decoder_tokenizer(cfg: CLaRaConfig) -> AutoTokenizer:
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            cfg.decoder_model_name, use_fast=True, padding_side="left"
        )
    ...
    def compress(
        self, enc_input_ids: torch.Tensor, enc_attention_mask: torch.Tensor
    ) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    def _compr_query_reasoner_stage2(
        self, input_ids: torch.Tensor, attention_mask: torch.Tensor
    ) -> torch.Tensor:
    ...
    def generate_from_questions(
        ...
        with torch.no_grad():
            ...
            if stage2_mips:
                ...
                scores = torch.bmm(
                    query_reps.unsqueeze(1), retrieved_doc_embeddings.transpose(1, 2)
                ).squeeze(1)
                ...
                input_encoder = self._prepare_encoder_inputs(
                    flat_documents, max_length=self.doc_max_length
                )
                ...
                retrieved_doc_embeddings = retrieved_doc_embeddings.reshape(
                    B, stage2_retrieval_top_n, -1
                )
                ...
                selected_doc_embeddings = torch.einsum(
                    "bkn,bnd->bkd", z.to(retrieved_doc_embeddings.dtype), retrieved_doc_embeddings
                )
                selected_doc_embeddings = selected_doc_embeddings.view(
                    selected_doc_embeddings.size(0) * selected_doc_embeddings.size(1),
                    -1,
            ...
            instructions = [
                self._blend_prompt_and_selected_memory_tokens(query=q)[1] for q in questions
            ]
        ...
        if time_count:
            return (
                ...
                query_time,
                generate_time,
                compress_time + query_time + generate_time,
            )
        else:
            return decoded, topk_idx

    def generate_from_paraphrase(
        self, questions: list[str], documents: list[list[str]], max_new_tokens: int = 128
    ) -> list[str]:
        """
        ...
        device = self.decoder.device
        model_input["enc_input_ids"], model_input["enc_attention_mask"] = (
        ...
        instr = [
            self._blend_prompt_and_memory_tokens(query="", stage="stage1", paraphrase_loss=True)
            for q in questions
        ]
        ...
        model_input["dec_input_ids"], model_input["dec_attention_mask"] = (
            inp_dec["input_ids"].to(device),
            inp_dec["attention_mask"].to(device),
        )
    ...
    def generate_from_text(
        ...
        instructions = [
            self._blend_prompt_and_memory_tokens(query=q, stage="stage1_2") for q in questions
        ]
        inp_dec = self.decoder_tokenizer(
            instructions,
            return_tensors="pt",
    ...
    def generate_from_compressed_documents_and_questions(
        self, questions: list[str], compressed_documents: torch.Tensor, max_new_tokens: int = 128
    ) -> list[str]:
    ...
    def _prepare_encoder_inputs(
        self, texts: list[str], max_length: int, q_texts: list[str] = None
    ...
    def _prepare_encoder_inputs_to_decoder(
        self, texts: list[str], max_length: int, q_texts: list[str] = None
    ) -> dict[str, torch.Tensor]:
        ...
        # For cross-encoder mode, don't add memory tokens (MHA compressor doesn't need them)
        if self.use_cross_encoder:
            return inp_enc
    ...
    def _replace_embeddings(
        ...
        for i in range(batch_size):
            for j in range(indices[i], indices[i + 1]):
                ...
                assert (
                    inputs_embeds[i, start_idx : start_idx + num_embs, :].size()
                    == compressed_embs[j].size()
                )
    ...
    def _retrieve_embeddings(
        self, questions: torch.Tensor, stage2_retrieval_top_n: int = 1
    ) -> torch.Tensor:
        ...
        retrieval_embeddings = torch.tensor(
            retrieval_embeddings, dtype=torch.bfloat16, device=questions.device
        )
    ...
    def save_pretrained(self, save_directory: str, **kwargs):
        ...
        if self.lora:
            ...
            torch.save(
                self._get_all_adapters_state_dict(), os.path.join(save_directory, "adapters.pth")
            )
        ...
        if config.lora:
            ...
            try:
                first_and_last_layers_path = hf_hub_download(
                    repo_id=pretrained_model_name_or_path, filename="decoder_first_last_layers.pth"
                )
            except Exception:
            ...
            try:
                adapters_path = hf_hub_download(
                    repo_id=pretrained_model_name_or_path, filename="adapters.pth"
                )
            except Exception:
                adapters_path = os.path.join(pretrained_model_name_or_path, "adapters.pth")

            if os.path.exists(adapters_path):
                adapters_state_dict = torch.load(
                    adapters_path, map_location=map_location, weights_only=True
                )
    ...
    def _load_adapters_from_state_dict(
        self, adapters_state_dict: dict, peft_config: LoraConfig, config: CLaRaConfig
    ):
        ...
        if not getattr(config, "pure_inference", False):
            for key, val in adapters_state_dict.items():
                ...
                if (
                    (config.training_stage == "stage1" and key == "query_reasoner_adapter")
                    or (
                        config.training_stage == "stage1_2"
                        and key
                        in [
                    ...
                    or (config.training_stage == "stage2_reasoning" and key == "decoder_adapter")
                ):
                    continue

                self._load_adapter_from_state_dict(
                    peft_config=peft_config, adapter_name=key, adapter_state_dict=val
                )
        else:
    ...
    def _load_adapter_from_state_dict(
        self, peft_config: LoraConfig, adapter_name: str, adapter_state_dict: dict
    ):
    ...
    def _handle_query_reasoner_adapter_loading(
        self, adapters_state_dict: dict, peft_config: LoraConfig
    ):
        """Handle special loading logic for query reasoner adapter."""
        if (
            "encoder_adapter" in adapters_state_dict
            and "query_reasoner_adapter" not in adapters_state_dict
        ):
    ...
    def _forward_batch(
        self, batch: dict, stage2_mips: bool, stage2_retrieval_top_n: int
    ) -> tuple[torch.Tensor, dict]:
    ...
    def _forward_stage2_batch(
        ...
        if stage2_mips:
            ...
            with torch.no_grad():
                retrieved_doc_embeddings, mse_loss = self.compress(
    ...
    def _forward_stage_1(
        ...
        decoder_outputs = self.decoder(
            inputs_embeds=inputs_embeds, attention_mask=dec_attention_mask, labels=labels
        )
    ...
    def _replace_reasoning_embeddings(
        self,
    ...
    def _generate(
        ...
        max_new_tokens: int = 128,
        return_doc_embeddings: bool = False,
    ) -> list[str]:
        ...
        if return_doc_embeddings:
            ...
            compressed_embs = compressed_embs.view(
                batch_size, top_k, compressed_embs.size(1), compressed_embs.size(2)
            )
