...
from openrlhf.utils.debug_utils import debug_clara_training
...
class RetrievalMetrics:
    ...
    def calculate_recall_and_precision(
        pos_indices: list[list[int]], pred_indices: list[list[int]], k_values: list[int] = [1, 3, 5]
    ) -> dict[str, float]:
...
class SFTTrainer(ABC):
    ...
    def _setup_logging(self):
        ...
        if self.strategy.args.use_tensorboard and self._wandb is None and self.strategy.is_rank_0():
            ...
            log_dir = os.path.join(
                self.strategy.args.use_tensorboard, self.strategy.args.wandb_run_name
            )
            self._tensorboard = SummaryWriter(log_dir=log_dir)

    def fit(self, args, consumed_samples: int = 0, num_update_steps_per_epoch: int | None = None):
        """
        Main training loop.
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for batch in self.train_dataloader:
                ...
                # Debug mode: show color-coded token confidence
                global_step = step // self.strategy.accumulated_gradient
                if (
                    getattr(args, "debug_mode", False)
                    and self.strategy.is_rank_0()
                    and global_step % getattr(args, "debug_every_steps", 10) == 0
                ):
                    try:
                        debug_clara_training(
                            model=self.model,
                            batch=batch,
                            outputs=outputs,
                            sample_idx=0,
                            max_print_tokens=getattr(args, "debug_max_print_tokens", 100),
                        )
                    except Exception as e:
                        print(f"[Debug] Warning: debug_clara_training failed: {e}")
                ...
                training_metrics = self._update_training_metrics(
                    training_metrics, step_metrics, loss, outputs
                )
    ...
    def _process_accumulated_step(
        ...
        if training_metrics["retrieval_samples"] > 0:
            for metric in [
                ...
                "retrieval_recall_3",
                "retrieval_recall_5",
                "retrieval_precision_1",
                "retrieval_precision_3",
    ...
    def save_logs_and_checkpoints(
        ...
        num_update_steps_per_epoch: int | None = None,
    ):
        """Save logs and checkpoints."""
        ...
        if global_step % args.logging_steps == 0:
            if self._wandb is not None and self.strategy.is_rank_0():
                logs = {
                    "train/%s" % k: v for k, v in {**logs_dict, "global_step": global_step}.items()
                }
    ...
    def _run_evaluation(self, args, global_step: int, num_update_steps_per_epoch: int | None):
        ...
        if global_step % (args.eval_steps * 5) == 0 and args.do_eval_gen:
            ...
            and global_step % num_update_steps_per_epoch == 0
            and args.do_eval_gen
        ):
    ...
    def evaluate(self, eval_dataloader, steps: int = 0, eval_gen: bool = False):
        ...
        with torch.no_grad():
            ...
            for batch in eval_dataloader:
                ...
                # Retrieval metrics
                if "stage2" in self.args.stage and "topk_idx" in outputs:
                    retrieval_metrics, valid_samples = (
                        RetrievalMetrics.calculate_recall_and_precision(
    ...
    def _generate_predictions(self, batch: dict[str, Any]) -> list[str]:
        ...
        if self.args.stage in ["stage1", "stage1_2"]:
            return self.model.generate_from_text(
                questions=questions, documents=docs, max_new_tokens=64
            )
    ...
    def _calculate_final_eval_metrics(
        self, eval_metrics: dict[str, float], eval_gen: bool
    ) -> dict[str, float]:
        ...
        if eval_metrics["retrieval_samples"] > 0:
            for metric in [
                ...
                final_metrics[f"eval_{metric}"] = (
                    eval_metrics[metric] / eval_metrics["retrieval_samples"]
                )
